...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # optim = torch.optim.SGD(self.model.parameters(), lr=lr, momentum=momentum, nesterov=True)
        ...
        train(self.model, self.trainloader, optim, epochs, self.device, type_fed='FedProx')
    ...
    # Configure logging
    with open('running_outputs/client_evaluation.log', 'w'):
        pass

    logging.basicConfig(filename='running_outputs/client_evaluation.log', level=logging.INFO, format='%(asctime)s - %(message)s')
    def evaluate(self, parameters: NDArrays, config: Dict[str, Scalar]):
        self.set_parameters(parameters)

        loss, accuracy = test(self.model, self.valloader, self.device)
        # Log the accuracy
        logging.info(f'Client evaluated with accuracy: {accuracy}')
