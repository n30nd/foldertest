...
def evaluate_metrics_aggregation_fn(metrics):
    ...
    print('################### EVALUATE METRIC AGGREGATION###################', sum(accuracies) / sum(examples))
    logging.info(f'EVALUATE_METRICS_AGGREGATION: {sum(accuracies) / sum(examples)}')
...
#     examples = [num_examples for num_examples, _ in metrics]
#     # Aggregate and return custom metric (weighted average)
#     print('################### FIT METRIC AGGREGATION###################', sum(accuracies) / sum(examples))
#     logging.info(f'FIT_METRICS_AGGREGATION: {sum(accuracies) / sum(examples)}')
#     return {"federated_fit_accuracy": sum(accuracies) / sum(examples)}
...
def get_evaluate_fn(num_classes: int, testloader):
    ...
    def evaluate_fn(server_round: int, parameters, config):
        ...
        print(f'Round {server_round} | Test Loss: {loss:.3f}, Test Acc: {accuracy:.3f}')
